...
export default function PaymentsPage() {
    ...
    const statusBadge = (status: string) => {
        ...
        return (
            <span
                className={`text-xs font-bold px-2.5 py-1 rounded-full ${map[status] ?? "bg-gray-100 text-gray-500"
                    }`}
            >
                {getTypeConfig(status).icon} {getTypeConfig(status).label}
            </span>
        );
    };
                        ...
                        <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-5">
                            ...
                            <div className="flex items-center gap-6 text-sm text-gray-600">
                                ...
                                <span className="bg-gray-50 px-2 py-0.5 rounded text-xs text-gray-500">
                                    {getTypeConfig(p.paymentType).icon} {getTypeConfig(p.paymentType).label}
                                </span>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-8 max-w-md mx-auto">
                            ...
                            <div className="bg-gray-50 rounded-xl p-4 mb-6 flex flex-col gap-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span>
                                        {getTypeConfig(selectedPayment.paymentType).icon}{" "}
                                        {getTypeConfig(selectedPayment.paymentType).label}
                                    </span>
                                </div>
                            </div>
